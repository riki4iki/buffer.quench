...
export default class CronService {
   ...
   private addListener(context: ICronnable) {
      ...
      const cron: Job = scheduleJob(`${context.id}`, context.expireDate, async function executer() {
         ...
         console.log(`${post.id} post to pages; ${ids} at time: ${post.expireDate} body: ${post.context}`);
